...
def _list_processes() -> list[dict]:
    ...
    with _PROCESS_LOCK:
        ...
        for ch in dead:
            del _PROCESSES[ch]
        for ch, proc in _PROCESSES.items():
            result.append({"channel": ch, "running": True, "pid": proc.pid})
    return result
...
def get_processes():
    return jsonify({"processes": _list_processes()})
...
def start_channel(channel: str):
    ...
    supported = {"telegram", "feishu", "qq", "imessage"}
    ...
    if channel == "telegram":
        ...
        if not token:
            return jsonify({"ok": False, "error": "Telegram token not configured — save config first"})
        cmd += ["--token", token]
        ...
        if not fc.get("app_id") or not fc.get("app_secret"):
            return jsonify({"ok": False, "error": "Feishu app_id/app_secret not configured"})
        cmd += ["--feishu-app-id", fc["app_id"], "--feishu-app-secret", fc["app_secret"],
                "--feishu-connection-mode", fc.get("connection_mode", "websocket")]
        ...
        cmd += ["--feishu-host", fc.get("host", "0.0.0.0"),
                "--feishu-port", str(fc.get("port", 8080)),
                "--feishu-path", fc.get("path", "/feishu/events")]

    elif channel == "qq":
        qc = cfg.get("qq", {})
        if not qc.get("app_id") or not qc.get("client_secret"):
            return jsonify({"ok": False, "error": "QQ app_id/client_secret not configured"})
    ...
    try:
        ...
        # bufsize=1 + text=True → line-buffered; the reader thread gets lines promptly
        ...
        # Pre-seed buffer with the command so it's visible even before any output
        _LOG_BUFFERS[channel].appendleft(f"$ {' '.join(cmd)}\n\n")
        return jsonify({"ok": True, "pid": proc.pid,
                        "message": f"Started {channel} bot (pid={proc.pid})"})
    except Exception as e:
